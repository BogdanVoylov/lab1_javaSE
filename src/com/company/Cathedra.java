...
public class Cathedra {
    ...
    public Student[] getStudentObjectsByYear(int year) {
        Student[] res = new Student[0];
        for (Student student : students) {
            if (year == student.getYear()) {
                res = Arrays.copyOf(res, res.length + 1);
                res[res.length - 1] = student.clone();
            }
        }
        return res;
    }
    ...
    public Teacher[] getTeachers() {
        return Arrays.copyOf(Resizer.clone(teachers), teachers.length, Teacher[].class);
    }
